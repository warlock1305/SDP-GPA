"""
Comprehensive Repository Analyzer v3.0
=====================================
...
Removed: Category classification (as requested)
Added: Real-time AST extraction, enhanced semantic analysis
"""
...
class ComprehensiveRepositoryAnalyzerV3:
    def __init__(self):
        ...
        print(f"🔧 Comprehensive Repository Analyzer v3.0 initialized")
    ...
    def analyze_architecture_patterns(self, features: Dict) -> Dict[str, Any]:
        """Analyze architecture patterns using enhanced analysis with significant dimensions."""
        patterns = {
            'detected_patterns': [],
            'pattern_confidence': {},
        ...
        # Enhanced pattern detection using significant dimensions
        category_scores = self._analyze_category_scores(features)
        patterns['category_analysis'] = category_scores
        
        # Analyze indicators for different patterns with enhanced logic
        language_counts = features.get('language_counts', {})
        total_source_files = features.get('total_source_files', 0)
        
        indicators = {
            'web_application': (
                features.get('web_development_keywords', 0) > 5 or 
                category_scores.get('web_application', 0) > 0.6 or
                language_counts.get('js', 0) > total_source_files * 0.3 or
                features.get('has_ci_cd', False) and features.get('has_dependencies', False)
            ),
            'data_science': (
                features.get('data_science_keywords', 0) > 5 or 
                category_scores.get('data_science', 0) > 0.6 or
                language_counts.get('py', 0) > total_source_files * 0.5
            ),
            'cli_tool': (
                features.get('cli_tool_keywords', 0) > 3 or 
                category_scores.get('cli_tool', 0) > 0.6 or
                (language_counts.get('js', 0) > 0 and features.get('total_files', 0) < 100)
            ),
            'mobile_app': (
                features.get('mobile_development_keywords', 0) > 3 or 
                category_scores.get('mobile_app', 0) > 0.6 or
                language_counts.get('py', 0) > total_source_files * 0.4 or
                language_counts.get('java', 0) > total_source_files * 0.3
            ),
            'game_development': (
                features.get('game_development_keywords', 0) > 3 or 
                category_scores.get('game_development', 0) > 0.6 or
                language_counts.get('js', 0) > total_source_files * 0.4 or
                language_counts.get('cpp', 0) > total_source_files * 0.3
            ),
            'library': (
                features.get('library_keywords', 0) > 5 or 
                category_scores.get('library', 0) > 0.6 or
                features.get('has_dependencies', False) and features.get('has_docs', False)
            ),
            'educational': category_scores.get('educational', 0) > 0.6,
            'microservices': features.get('has_docker', False) and features.get('has_ci_cd', False),
            'monolithic': features.get('total_files', 0) > 100 and not features.get('has_docker', False)
        }
        ...
        for pattern, indicator in indicators.items():
            if indicator:
                ...
                confidence = self._calculate_pattern_confidence(pattern, features, category_scores)
                patterns['pattern_confidence'][pattern] = confidence
        
        # Enhanced architectural indicators
        patterns['architectural_indicators'] = {
    ...
    def _analyze_category_scores(self, features: Dict) -> Dict[str, float]:
        """Analyze category scores using significant dimensions."""
        category_scores = {}
        
        # Analyze significant dimensions for category classification
        significant_dims = {k: v for k, v in features.items() if k.startswith(('cli_tool_vs_data_science_dim_', 'web_application_vs_library_dim_', 'game_development_vs_mobile_app_dim_', 'data_science_vs_web_application_dim_'))}
        
        # Calculate scores based on significant dimensions
        if significant_dims:
            # CLI vs Data Science analysis
            cli_ds_dims = {k: v for k, v in significant_dims.items() if k.startswith('cli_tool_vs_data_science_dim_')}
            if cli_ds_dims:
                cli_score = sum(v for k, v in cli_ds_dims.items() if 'dim_448' in k or 'dim_720' in k)
                ds_score = sum(v for k, v in cli_ds_dims.items() if 'dim_644' in k or 'dim_588' in k)
                category_scores['cli_tool'] = max(0, cli_score / len(cli_ds_dims))
                category_scores['data_science'] = max(0, ds_score / len(cli_ds_dims))
            
            # Web Application vs Library analysis
            web_lib_dims = {k: v for k, v in significant_dims.items() if k.startswith('web_application_vs_library_dim_')}
            if web_lib_dims:
                web_score = sum(v for k, v in web_lib_dims.items() if 'dim_588' in k or 'dim_498' in k)
                lib_score = sum(v for k, v in web_lib_dims.items() if 'dim_720' in k or 'dim_77' in k)
                category_scores['web_application'] = max(0, web_score / len(web_lib_dims))
                category_scores['library'] = max(0, lib_score / len(web_lib_dims))
            
            # Data Science vs Web Application analysis (NEW)
            ds_web_dims = {k: v for k, v in significant_dims.items() if k.startswith('data_science_vs_web_application_dim_')}
            if ds_web_dims:
                # Use the most significant dimensions from our analysis
                ds_favored_dims = [23, 177, 430, 468, 545, 571, 618, 660]  # Positive t-stats favor DS
                web_favored_dims = [104, 132, 134, 164, 346, 360, 495, 504, 506, 558, 645, 767]  # Negative t-stats favor Web
                
                ds_score = sum(v for k, v in ds_web_dims.items() if any(f'dim_{d}' in k for d in ds_favored_dims))
                web_score = sum(v for k, v in ds_web_dims.items() if any(f'dim_{d}' in k for d in web_favored_dims))
                
                # Normalize scores
                total_ds_dims = len([k for k in ds_web_dims.keys() if any(f'dim_{d}' in k for d in ds_favored_dims)])
                total_web_dims = len([k for k in ds_web_dims.keys() if any(f'dim_{d}' in k for d in web_favored_dims)])
        ...
        if not category_scores:
            category_scores = {
                ...
                'educational': 0.0  # Would need specific educational keywords
            }
        
        return category_scores
    
    def _calculate_pattern_confidence(self, pattern: str, features: Dict, category_scores: Dict) -> float:
        """Calculate confidence score for a detected pattern."""
        base_confidence = 0.5
        
        # Adjust confidence based on category scores
        if pattern in category_scores:
            base_confidence += category_scores[pattern] * 0.3
        
        # Adjust based on keyword presence
        keyword_map = {
            'web_application': 'web_keywords',
            'data_science': 'data_science_keywords',
            'cli_tool': 'cli_keywords',
            'mobile_app': 'mobile_keywords',
            'game_development': 'game_keywords',
            'library': 'library_keywords'
        }
        
        if pattern in keyword_map:
            keyword_count = features.get(keyword_map[pattern], 0)
            base_confidence += min(0.2, keyword_count / 20.0)
        
        # Adjust based on semantic coherence
        if features.get('semantic_coherence', 0) > 0.5:
            base_confidence += 0.1
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python comprehensive_repository_analyzer_v3.py <repository_path>")
        sys.exit(1)
